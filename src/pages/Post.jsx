...
for(let i=0; i<UsersData.length; i++){
    post = UsersData[i].posts.find(post=> post.id == id);
...
<div className="flex gap-4 items-center mb-3">
    <img src={user.image} alt="" className="h-15 w-15 rounded-full object-cover" />
    <div className="">
        <p className="m-0">{user.name}</p>
        <p className="m-0 text-sm text-neutral-600">{user.designation}</p>
    </div>
</div>


<img src={post.pic} alt="" className="" />
...
<p className="">{(new Date(post.date)).toLocaleDateString('en', options )}</p>
</div>
